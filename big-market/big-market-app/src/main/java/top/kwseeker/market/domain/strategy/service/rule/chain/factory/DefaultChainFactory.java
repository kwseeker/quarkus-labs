package top.kwseeker.market.domain.strategy.service.rule.chain.factory;
...
public class DefaultChainFactory {
    ...
    public ILogicChain openLogicChain(Long strategyId) {
        ...
        if (null == ruleModels || 0 == ruleModels.length) {
            ...
            ILogicChain ruleDefaultLogicChain = CDIBeanSelector.select(ILogicChain.class, LogicModel.RULE_DEFAULT.getCode());
            // 写入缓存
        ...
        ILogicChain logicChain = CDIBeanSelector.select(ILogicChain.class, ruleModels[0]);
        ILogicChain current = logicChain;
        for (int i = 1; i < ruleModels.length; i++) {
            //ILogicChain nextChain = applicationContext.getBean(ruleModels[i], ILogicChain.class);
            ILogicChain nextChain = CDIBeanSelector.select(ILogicChain.class, ruleModels[i]);
            current = current.appendNext(nextChain);
        }

        // 责任链的最后装填默认责任链
        //current.appendNext(applicationContext.getBean(LogicModel.RULE_DEFAULT.getCode(), ILogicChain.class));
        current.appendNext(CDIBeanSelector.select(ILogicChain.class, LogicModel.RULE_DEFAULT.getCode()));
        // 写入缓存
